...
pub fn view<'a, 'b, C: GoodPixelColor, F: Fn(&State) + 'a + Copy>(
    ...
    .map_event(|event: &Event, _state| match *event {
        Event::KeyDown { key, group } => match key {
            Key::Character('h') => Mapping::Fallback(Event::KeyDown {
                key: Key::LeftArrow,
                group,
            }),
            Key::Character('l') => Mapping::Fallback(Event::KeyDown {
                key: Key::RightArrow,
                group,
            }),
            Key::Character('k') => Mapping::Fallback(Event::KeyDown {
                key: Key::UpArrow,
                group,
            }),
            Key::Character('j') => Mapping::Fallback(Event::KeyDown {
                key: Key::DownArrow,
                group,
            }),
